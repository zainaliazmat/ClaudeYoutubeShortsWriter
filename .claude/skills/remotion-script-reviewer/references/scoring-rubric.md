...
| 4 | **Text & Caption Clarity** | 11 | Caption style + generation method named — **word-by-word from `vo-timing.json` integer word frames** (the VO default; not Whisper, not hand-typed windows), or hand-timed to beats for a no-VO Short; text is legible (short fragments, high contrast); typewriter effects use slicing not per-char opacity. |
| 5 | **Audio Sync** | 9 | Track vibe specified; **the voiceover (VO) is the LEAD and the music bed DUCKS under it** (bed ~0.72 → ~0.22 during speech, swelling on the payoff), driven by the `vo-timing.json` `speech_regions` envelope; **the mix note names the -14 LUFS / ≤ -1 dBTP master target**; every SFX has a frame number matching a visual event. A VO buried under the bed, a bed that never ducks, or a missing master target loses points. **No-VO special case:** if the Short has no voiceover, the music bed is itself the LEAD (mix it loud, never "under" anything) — the old rule. Either way a bed mixed too quiet / a missing master target ships near-silent in-feed (YouTube won't boost it). A deliberate silent piece: score on intent, but note it. |
