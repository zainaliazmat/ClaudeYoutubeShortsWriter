...
You are running the `/short` agentic pipeline. **Read `CLAUDE.md` and `memory/lessons.md` FIRST**, then follow the CLAUDE.md pipeline section exactly, in order. Use a TodoWrite list mirroring the 7 steps so progress is visible.
...
- Step 6 is the evaluator-optimizer loop (max 3 iterations) using the `remotion-script-reviewer` skill.
